...
router.get('/:symbol/decisions', (req, res) => {
  try {
    ...
    const raw = getDecisionsBySymbol(symbol, limit);
    const decisions = raw.map(d => ({
      id:         d.id,
      timestamp:  new Date(d.timestamp).toISOString(),
      symbol:     d.symbol,
      price:      d.price,
      marketMode: d.market_mode ?? d.marketMode,
      decision:   d.decision,
      reason:     d.reason
    }));
...
router.post('/:symbol/news/refresh', async (req, res) => {
  try {
    const symbol = req.params.symbol.toUpperCase();
    if (symbol === 'PAXG') {
      return res.status(400).json({ error: 'PAXG usa /gold-context/refresh' });
    }
    if (!isValidSymbol(symbol)) {
      return res.status(400).json({ error: 'Símbolo no válido' });
    }
